...
SAMPLE_DIR = Path("data/samples/list")
...
def _load_first_real_sample(sample_dir: Path) -> str:
    ...
    if not candidates:
        pytest.skip(f"No real list snapshot sample found in {sample_dir}")
    return candidates[0].read_text(encoding="utf-8", errors="replace")


def test_extract_list_from_real_local_sample_without_network():
